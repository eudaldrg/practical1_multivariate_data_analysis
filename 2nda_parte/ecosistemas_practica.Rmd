...
```{r}
fit.l.cv <- lda(cluster.km.2 ~ ., data = data, na.action = "na.omit", CV = T) # Linear and crossvalidated
...
# element <- element - apply(data[,-8], MARGIN = 2, mean)
# element <- element / apply(data[,-8], MARGIN = 2, sd)

# data$cluster <- as.factor(result.km.2$cluster)
fit.l.ncv <- lda(cluster.km.2 ~ ., data = data, na.action = "na.omit", CV = F) # Should use the non cv for prediction and the cv for analysis of the discriminant itself
# ct <- table(data$cluster, predict(fit.l.ncv,data[, -8])$class) 
# sum(diag(prop.table(ct)))
predict(fit.l.ncv, element)

# fit.q.ncv <- qda(cluster ~ ., data = data, na.action = "na.omit", CV = F)
# sum(diag(prop.table(table(data$cluster, predict(fit.q.ncv, data[,-8])$class))))
# predict(fit.q.ncv, element)

# fit.q.cv <- qda(cluster ~ ., data = data, na.action = "na.omit", CV = T)
# sum(diag(prop.table(table(data$cluster, fit.q.cv$class))))
```