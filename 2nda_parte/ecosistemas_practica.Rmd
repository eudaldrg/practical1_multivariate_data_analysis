...
ecosystems<-read.table("./ecosystems_windows.txt",sep="\t",header=T)
```
...
``` {r echo = F, warning = F, message = F, error = F}
round(cor(ecosystems), 3)[1:3, 1:3]
```
...
# nrow(ecosystems)
# head(ecosystems, 5)
countries2 <- scale(ecosystems)
## Matriz de interdistancias (Parte 1)
ecosystems.D1 <- dist(countries2, method = "euclid")
head(as.matrix(dist(countries2, method = "euclid", upper = T))[,1:5], 5)
```
...
plot(clusterW, hang=-1, labels = ecosystems[,1], main = "Cluster Dendogram (Ward)", cex = 0.5)

clusterUPGMA <- hclust(ecosystems.D1, method = "average")
plot(clusterUPGMA, hang=-1, labels = ecosystems[,1], main = "Cluster Dendogram (UPGMA)", cex = 0.5)
```
...
Below are shown the values of the representatives of each of the two groups. The analysis has been done with standardized values, so the representatives are shown accordingly. It can be seen that the first group representative has relatively low values in all the contamination indicators. The group 2 representative has overall positive values (which mean higher than the mean values in the original data), in particular for the organic contamination variables mentioned at the beginning and in __DQO_M__ (which also determines organic contamination). Hence, group 1 represents low contamination and group 2 represents high organic contamination. This will be expanded further when analizing the groups in a 2 dimentional space, but we don't feel confident enough yet to infer anything about the inorganic contamination.
...
g1<-as.matrix(ecosystems[which(ecosystems$cluster==1),-8]) # Multivariate normality
AD.test(g1, qqplot = FALSE)

g2<-as.matrix(ecosystems[which(ecosystems$cluster==2),-8]) # Multivariate normality
AD.test(g2, qqplot = FALSE)
```

As they follow multivariate normals, we can test for the homogeneity of their Variance/Covariance matrices and their mean vectors. As seen in the chi-squared test below, the p-value for the homogeneity of the variance-covariance matrix is really high, so we cannot reject the null hypothesis of the variance-covariance matrix being homogenious.

```{r}
boxM(ecosystems[,-8], ecosystems[,8])
```
...
# element <- element - apply(ecosystems[,-8], MARGIN = 2, mean)
# element <- element / apply(ecosystems[,-8], MARGIN = 2, sd)

# ecosystems$cluster <- as.factor(result.km.2$cluster)
fit.l.ncv <- lda(cluster.km.2 ~ ., data = ecosystems, na.action = "na.omit", CV = F) # Should use the non cv for prediction and the cv for analysis of the discriminant itself
# ct <- table(ecosystems$cluster, predict(fit.l.ncv,ecosystems[, -8])$class) 
# sum(diag(prop.table(ct)))
predict(fit.l.ncv, element)

# fit.q.ncv <- qda(cluster ~ ., data = ecosystems, na.action = "na.omit", CV = F)
# sum(diag(prop.table(table(ecosystems$cluster, predict(fit.q.ncv, ecosystems[,-8])$class))))
# predict(fit.q.ncv, element)

# fit.q.cv <- qda(cluster ~ ., data = ecosystems, na.action = "na.omit", CV = T)
# sum(diag(prop.table(table(ecosystems$cluster, fit.q.cv$class))))
```
